...
parser.add_argument('--config_file', type=str, default='config_template_Skin_3.yaml')
args = parser.parse_args()

def infer_single_slide(model, features, label, reverse_label_dict, k=1):
	features = features.to(device)
	with torch.no_grad():
		if isinstance(model, (CLAM_SB, CLAM_MB,TOAD_fc_mtl_concat)):
...
if __name__ == '__main__':
	config_path = os.path.join('D:\\Project\\IBD\\CLAM\\heatmaps\\configs', args.config_file)
